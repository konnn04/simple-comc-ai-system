...
def save_listening_exam_to_db(user_id, course_id, audio_path, questions, is_ai=True, is_public=True, score=10.0):
    """
    Save a listening exam and its questions to the database
    ...
    try:
        ...
        db.session.commit()
        return {'success': True, 'exam_id': listening_exam.id}
    except Exception as e:
        db.session.rollback()
